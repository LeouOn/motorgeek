...
def _dimensional_avg(rel) -> float | None:
    """Average of available dimensional scores, or None."""
    if rel is None:
        return None
    vals = [v for v in [
        rel.score_engine, rel.score_transmission, rel.score_chassis,
        rel.score_electronics, rel.score_ease_of_repair,
    ] if v is not None]
...
def build_radar_data(cars: list, perf_data: dict, ice_data: dict, rel_data: dict, cost_data: dict) -> dict:
    ...
    for i, car in enumerate(cars):
        ...
        rel_score = _dimensional_avg(rel) or (rel.reliability_score if rel and rel.reliability_score else 50)
